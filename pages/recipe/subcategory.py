...
def subcategory_table(category:str, subcategory:str) -> str: 
    body = f'''
    <h1>{category.capitalize()}/{subcategory.capitalize()}</h1>
    <table class="table table-striped table-hover table-sm">
        ...
        <tbody>
    '''
    recipes = db.query(f"SELECT * FROM recipes WHERE category = '{category.replace("'", "''")}' AND subcategory = '{subcategory.replace("'", "''")}'")
    for r in recipes:
